...
?> 
 ...
 <div style="position: relative;  background-image: url(bg_transparent.png); z-index: 4; left: 147px; top: -20px">

 	<? 
function getBrc($data){ $kk3=0;
	//Проверяем что массив не пуст
	if (empty($data)) {  return false; } else {
		$brc = $data;
		$brc_menu = ''; 
		//Перебераем массив для построения хлебных крошек
			foreach ($brc as $id => $title) { $kk3++;
